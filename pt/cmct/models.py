...
class Seed(models.Model):
    ...
    size = models.FloatField(default=0) # GB
    seeders = models.IntegerField(default=0)
    user_as = models.ManyToManyField(User, through='Assign')
    user_ac = models.ManyToManyField(User, through='Accept')

    def __str__(self):
        return f'<Seed - {self.id} as:{self.seed_as.count()} ac:{self.seed_ac.count()} \
se:{self.seed_se.count()}>'
...
class Assign(models.Model):
    ...
    def __str__(self):
        return f'<Assign - {self.seed.id}:{self.user.name}>'
...
class Secure(models.Model):
    ...
    def __str__(self):
        return f'<Secure - {self.seed.id}:{self.user.name}>'
